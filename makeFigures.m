...
%     outFile = [ outDir, '/', strjoin(nameParts(1:end-1),'_'), '.png' ];
...
%     reconImg = imread( [inDir, '/', reconName] );
%     N = size(reconImg);
...
%     imwrite( scaledImg, [outDir, '/', scaledName] );
% 
%     close all; drawnow;
%   end
  ...
  wFiles = dir( [inDir, '/weights_*.mat'] );
  for i=1:numel(wFiles)
    rlsdcMatch = regexp(wFiles(i).name, 'LSDC_con');
    lsdcMatch = regexp(wFiles(i).name, 'LSDC');
  ...
  set(gca,'fontsize',20);
  legend( 'LSDC', 'rLSDC' );
  saveas( noiseH, [outDir, '/noiseSensitivity.png'] );
