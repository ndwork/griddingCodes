function im = gridifft2( d, k, n, varargin )
% GRIDIIFT3 Compute the three-dimensional inverse-DFT via gridding
%   IM = GRIDIFFT2( D, K, N, [A, L, C, P, T] ) grids the data D to the
...
%       A: the oversampling ratio; <--------------------------------------- hacked! do not make this anisotropic (i.e. non-square image)! -
...
%
%   GRIDIFFT2 uses GRIDMR and IFFT2C.
%
% Ethan Johnson, 2014
...
im = gridmr( d, k, aln(1), s, a ); clear d; % <---------------------------- hack!! (ignore non-square image request) -
...
% De-apodise
if f_time, time=tic; end
[x,y] = meshgrid( ((-aln(2)/2):(aln(2)/2-1))/aln(2), ...
                  ((-aln(1)/2):(aln(1)/2-1))/aln(1) );
switch l
    case 1
        deap = (sinc((s-1)/2*x).*sinc((s-1)/2*y)).^2;
    case 2
        be = pi^2*((s/al*(al-0.5))^2 - 0.8); % (optimal shape parameter)^2
        t = (pi*s*x).^2-be; u = (pi*s*y).^2-be;
        deap = sinc(sqrt(t)/pi) .* sinc(sqrt(u)/pi);
end
deap(deap<thr) = thr;
